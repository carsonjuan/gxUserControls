...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'MenuMagico'">
        <xsl:call-template name="RenderMenuMagico"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="AddStyleAttribute" >
    <xsl:variable name="Style">
      ...
      <xsl:text>248</xsl:text>
      <xsl:text>; </xsl:text>
      <xsl:text>height: </xsl:text>
      <xsl:text>324</xsl:text>
      <xsl:text>; </xsl:text>
